...
public:
	...
	[[nodiscard]]
	size_t wait(TLock& lock, TPredicate&& pred) const
	{
	...
	[[nodiscard]]
	std::optional<size_t> wait_for(TLock& lock, const std::chrono::duration<TRep, TPeriod>& rel_time, std::optional<TPredicate>&& pred = {}) const
	{
		if (pred)
	...
	[[nodiscard]]
	std::optional<size_t> wait_until(TLock& lock, const std::chrono::time_point<TClock, TDuration>& timeout_time, std::optional<TPredicate>&& pred = {}) const
	{
		if (pred)
