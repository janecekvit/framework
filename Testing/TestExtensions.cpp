...
public:
	...
	TEST_METHOD(TestResourceWrapper)
	{
		...
		{ // wrapper scope -> Copy constuctor janecekvit::extensions::resource_wrapper
			auto oWrapperInt = janecekvit::extensions::resource_wrapper<int*>(new int(5), [&uLambdaCalled](int*& i)
				{
					delete i;
					i = nullptr;
			...
			oWrapperInt = std::move(janecekvit::extensions::resource_wrapper<int*>(new int(10), [&uLambdaCalled](int*& i)
				{
					delete i;
					i = nullptr;
					uLambdaCalled++;
			...
			auto oWrapperInt2 = janecekvit::extensions::resource_wrapper<int*>(new int(15), [&uLambdaCalled, &bCopyDestruction](int*& i)
				{
					if (bCopyDestruction)
						return;
		...
		// std namespace test
		auto oWrapperFile = janecekvit::extensions::resource_wrapper<std::fstream>(std::fstream("Test", std::ios::binary), [](std::fstream& i)
			{
				i.close();
			});
		...
		{ //Copy constructible
			...
			auto oWrapperUniqueInt = janecekvit::extensions::resource_wrapper<std::unique_ptr<int>>(std::make_unique<int>(5), [](std::unique_ptr<int>& i)
				{
					i.release();
				});

			auto oWrapperString = janecekvit::extensions::resource_wrapper<std::string>("BLSA", [](std::string& i)
				{
					i.clear();
				});

			auto oWrapperString2 = janecekvit::extensions::resource_wrapper<std::string>("DRDF", [](std::string& i)
				{
					i.clear();
				});
		...
		{ //Assigment operator reset
			...
			auto oTestOperatorReset2 = janecekvit::extensions::resource_wrapper<int*>(iTest2, [&](int*& i)
				{
					if (*i == 10)
						uDestroyed2++;
