...
public:
	...
	[[nodiscard]]
	bool wait_for(TLock& lock, const std::chrono::duration<TRep, TPeriod>& rel_time, std::optional<TPredicate>&& pred = {}) const
	{
		if (pred)
		{
	...
	[[nodiscard]]
	bool wait_until(TLock& lock, const std::chrono::time_point<TClock, TDuration>& timeout_time, std::optional<TPredicate>&& pred = {}) const
	{
		if (pred)
		{
